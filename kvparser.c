...
static int add_kvpair(struct kvparser* parser, kvlist* list, int state, char* key_buff, size_t key_len, char* val_start, char* val_end) {
	int err = 0;
	ssize_t process_len;
	struct kvpair* pair;
...
static char* strnstr(char* haystack, char* needle, size_t n) {
...
int kvparser_parse_string(struct kvparser* parser, kvlist* pairs, char* str, size_t len) {
	...
	while(parse_pos < parse_limit) {
		...
		if(!strnmincmp(parse_pos, parser->sep, parse_limit - parse_pos)) {
			...
			if((err = add_kvpair(parser, &kvpairs, state, key_buff, key_len, last_token, parse_pos))) {
				goto fail;
			}
	...
	if((err = add_kvpair(parser, &kvpairs, state, key_buff, key_len, last_token, parse_limit))) {
		goto fail;
	}
